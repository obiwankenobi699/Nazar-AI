...
export default function Page() {
  ...
  const frameCounterRef      = useRef(0)       // counts analyzeFrame calls for embedding
  const lastForcedGptRef     = useRef(0)       // timestamp of last forced GPT call
  ...
  const analyzeFrame = async () => {
    ...
    // Capture timestamp NOW (before async GPT call) — fixes the timestamp bug
    ...
    // ── 30s forced GPT fallback — catches silent collapses ──────────────
    const now30 = Date.now()
    const forcedGpt = now30 - lastForcedGptRef.current > 30000
    if (forcedGpt) {
      lastForcedGptRef.current = now30
      console.log('[FILTER] ⏰ 30s forced GPT call')
    }

    const { send, score, reason, debug } = frameFilter.evaluate(canvasRef.current, hints)
    const shouldSend = send || forcedGpt
    ...
    // ── Every 4th frame → send to embedder (async, non-blocking) ────────
    if (frameCounterRef.current % 4 === 0 && lastPoseKeypoints.length > 0) {
      captureFrame().then(f => {
        if (f) {
           fetch('/api/embed', {
             method: 'POST',
             ...
             body: JSON.stringify({
               ...
               cameraId: 'cam-0',
               description: transcriptRef.current || 'No description',
             }),
           }).catch(() => {}) // fire-and-forget, embedder may not be running
        }
      })
    }
...
try {
  ...
  for (const event of result.events) {
    ...
    // Embed every event frame (dangerous or suspicious)
    if (event.isDangerous || recordingStateRef.current === 'SUSPICIOUS') {
      console.log('[EMBED] Sending event frame to embedder:', {
        description: event.description,
        timestamp: capturedTimestamp,
        isDangerous: event.isDangerous,
        isSuspicious: recordingStateRef.current === 'SUSPICIOUS',
      })
      fetch('/api/embed', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          timestamp: capturedTimestamp,
          cameraId: 'cam-0',
          description: event.description || transcriptRef.current || 'No description',
        }),
      })
      .then(async (res) => {
        if (res.ok) {
          console.log('[EMBED] Frame successfully embedded.')
        } else {
          const err = await res.text()
          console.error('[EMBED] Embedder returned error:', err)
        }
      })
      .catch((err) => {
        console.error('[EMBED] Failed to call embedder:', err)
      })
    } else {
      console.log('[EMBED] Event not embedded (not dangerous/suspicious):', event.description)
    }
